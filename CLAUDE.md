...
11. **Cambio al flujo de feedback** (botón 👎, `feedback.jsonl`,
    `/api/feedback`)? → Update `docs/web-html/PRD.md` sección "Curación
    de datos" + CLAUDE.md sección "Feedback desde el modal".
...
data/
  ...
  feedback.jsonl                     — gitignored. Feedback que el usuario
                                       dejó vía el botón 👎 del dashboard.
                                       Contiene todos los campos del item
                                       original + `reason` + `submitted_at`.
...
scripts/
  ...
  retrofit/                          — utilities to apply changes to
    ...
    upgrade_image_resolution.py      — re-descarga portadas en resolución
                                       completa eliminando parámetros de
                                       redimensionado de CDN. Detecta 3
                                       patrones: Magento query params
                                       (?quality=80&width=222&height=222),
                                       WordPress sufijos (-300x300.jpg),
                                       Shopify sufijos (_540x.jpg). Compara
                                       dimensiones de píxeles antes de
                                       reemplazar (--min-gain 0.10 = 10%
                                       más píxeles mínimo). Actualiza
                                       image_url e images[] en items.jsonl.
                                       Las miniaturas viejas quedan como
                                       huérfanos para el próximo GC de
                                       mirror_images.py. Idempotente.
    backfill_prh_covers.py           — portadas EN via CDN de Penguin Random
                                       House. Para items EN con ISBN-13
                                       (prefijos 978-0/978-1) que no usan aún
                                       PRH CDN, prueba
                                       images.penguinrandomhouse.com/cover/{isbn13}.
                                       Valida ≥80 000 px (descarta placeholders).
                                       Compara con la imagen actual por píxeles
                                       (--min-gain 0.10). Deduplicado por ISBN.
                                       Idempotente. 18 items mejorados en la
                                       corrida inicial (2026-05-28).
    upscale_images.py                — AI upscaling de portadas pixeladas.
                                       Para imágenes con menos de --max-pixels
                                       px (default 200 000 ≈ 450×445),
                                       aplica waifu2x-ncnn-vulkan (preferido)
                                       o realesrgan-ncnn-vulkan ×2 y guarda el
                                       resultado como PNG lossless. Actualiza
                                       image_local en items.jsonl si la
                                       extensión cambia .jpg → .png.
                                       Idempotente. Requiere instalar:
    ...
    set_rarity.py                    — aplica el campo `rarity` a todos
                                       los items usando derive_rarity_tier().
                                       Solo toca items sin valor asignado
                                       (o todos con --force). Respeta valores
                                       asignados por web-search (p. ej.
    ...
    translate_descriptions.py        — traduce description → description_es
                                       ...
                                       deepl.
    fetch_better_covers.py           — busca portadas en mayor resolución
                                       para items con imagen pequeña
                                       (< --min-pixels, default 100 000 px).
                                       Estrategias por orden de prioridad:
                                       (1) ISBN → Amazon CDN + PRH CDN (EN)
                                       + OpenLibrary (gratis, sin cuota).
                                       (2) Sin ISBN → Brave Search API
                                       (BRAVE_API_KEY, 2 000 queries/mes,
                                       preferido) o Tavily (TAVILY_API_KEY,
                                       1 000 queries/mes, fallback). Si hay
                                       ambas keys, Brave va primero. Keys
                                       auto-detectadas desde .env.
                                       Verificación: perceptual hash aHash
                                       8×8, distancia Hamming ≤
...
.claude/skills/
  ...
  review-feedback/SKILL.md           — Skill manual: lee data/feedback.jsonl
                                       (feedback via 👎 del dashboard; ya
                                       contiene campos completos del item +
                                       reason + submitted_at). Categoriza
                                       causa raíz con 14 categorías: A–J
                                       (problemas de filtro: merch, trading
                                       cards, noticias, tomo regular, source
                                       ruidosa, western comic, LN, preferencia
                                       personal, falsa señal, selector amplio)
                                       + K–N (calidad de datos: imagen
                                       equivocada, metadata incorrecta,
                                       series_key/edition_key mal asignado,
                                       título con basura). Presenta propuestas
                                       al usuario, aplica aprobadas con tests
                                       + retrofits, y trunca la queue.
web/
...
### Backups en retrofit scripts — siempre con `backup_and_rotate()`

**Regla:** todo script que modifique `data/items.jsonl` (u otro archivo de
datos) **DEBE** usar `backup_and_rotate()` importada de `manga_watch.py`.
NUNCA con lógica propia ni con `cp ... /tmp/`.

```python
from manga_watch import backup_and_rotate   # al top del archivo

# Una sola vez, ANTES del loop:
if not dry_run:
    backup_and_rotate(items_path, "nombre-del-script")
```

`backup_and_rotate(path, label, max_keep=3)` escribe el backup en
**`data/backups/<filename>/`** (ej. `data/backups/items.jsonl/items.nombre-20260530.bak`)
y rota automáticamente, conservando solo los 3 más recientes.

**Por qué no inventar lógica propia:**
- `data/backups/` está en `.gitignore`. Un path calculado erróneamente
  (p. ej. `items_path.parent.parent / "backups"`) crea `backups/` en la
  raíz del repo — aparece en `git status` y no tiene rotación.
- La experiencia demostró que cada implementación propia diverge
  (`fetch_better_covers.py` usó exactamente ese path erróneo hasta 2026-05-30).

**Anti-patterns:**
- ❌ `cp data/items.jsonl /tmp/items.jsonl.bak-$(date)`
- ❌ `backup_dir = items_path.parent.parent / "backups"` (hardcoded incorrecto)
- ❌ Definir `_backup_items()` local en el script en vez de importar

Todos los retrofits existentes ya usan `backup_and_rotate()`. Si agregás
uno nuevo, hacé lo mismo desde el primer commit.

### Flush incremental — obligatorio en scripts de larga duración

**Regla:** todo script que procese items en un loop de red o subprocess
(HTTP fetch por ítem, Tavily API, waifu2x subprocess, Wayback API, etc.)
**DEBE escribir `items.jsonl` de forma incremental** — ya sea después de
cada mejora o cada N ítems. Un write único al final es inaceptable en
scripts que pueden tardar >2 minutos: si el proceso se cancela (Ctrl+C,
OOM, timeout del sistema), se pierden tanto el avance como los créditos
de API ya consumidos.

**Cuándo aplica (scripts de red/subprocess — siempre flush incremental):**
| Script | Patrón actual |
|---|---|
| `fetch_better_covers.py` | flush por cada mejora |
| `upgrade_image_resolution.py` | flush cada 50 mejoras |
| `backfill_prh_covers.py` | flush cada 20 mejoras |
| `mirror_images.py` | flush cada 200 URLs procesadas |
| `upscale_images.py` | flush por cada upscale exitoso |
| `wayback_recover.py` | flush cada 10 recuperaciones |
| `backfill_metadata.py` | flush cada N ítems (`--flush-every`, default 50) |
| `translate_descriptions.py` | flush cada N ítems (`--flush-every`, default 50) |

**Cuándo NO aplica (compute-only — write único al final OK):**
`rescore.py`, `clean_titles.py`, `filter_non_manga.py`,
`filter_collectible.py`, `backfill_cluster_key.py`, `generate_slugs.py`.
Estos son operaciones en memoria pura que terminan en segundos.

**Patrón de implementación** para scripts de red secuenciales:
```python
# Backup ANTES del loop (no al final)
if not dry_run:
    backup_and_rotate(items_path, "my-script")

for item in candidates:
    result = do_network_thing(item)
    if result:
        apply_to_item(item, result)
        if not dry_run:
            _write_items(items_path, items)  # flush inmediato

# Flush final (garantiza el último bloque)
if not dry_run:
    _write_items(items_path, items)
```

**Para scripts paralelos** (ThreadPoolExecutor): flush cada N mejoras
dentro del `as_completed` loop. El backup también va antes del pool.

**Para scripts con `--flush-every`** (backfill_metadata, translate_descriptions):
el flag ya controla la frecuencia. El default de 50 es razonable para
llamadas HTTP rápidas; para llamadas lentas (Wayback, waifu2x) usar 5-10.

**Importante**: el backup (`backup_and_rotate`) siempre se hace UNA VEZ
antes del loop, no dentro. Un flush incremental es `_write_items` directo
(sin backup) porque el backup inicial ya protege el estado original.

### Cómo lanzar scripts de larga duración — siempre con `nohup`

**Regla:** todo script que tarde más de ~2 minutos se lanza con `nohup`
para que sobreviva cierres de terminal, resets de sesión de Claude y
compactaciones de contexto. El proceso del harness de Claude Code se
puede cortar cuando el contexto se llena — un script lanzado sin `nohup`
muere con él.

**Forma canónica:**
```bash
nohup .venv/bin/python -u scripts/retrofit/<script>.py [flags] \
  > logs/<script>_run.log 2>&1 &
echo "PID: $!"
```

- **`nohup`** — ignora SIGHUP (cierre de terminal/sesión).
- **`-u`** — Python unbuffered: el log se escribe en tiempo real, no al final.
- **`> logs/... 2>&1`** — stdout + stderr al log; NO usar `tee` (buffering).
- **`&`** — background inmediato, libera el prompt.
- **`echo $!`** — reporta el PID para poder monitorearlo o matarlo.

**Comandos de monitoreo** (en cualquier terminal, en cualquier momento):
```bash
tail -f logs/<script>_run.log      # progreso en tiempo real
ps aux | grep <script>             # ¿sigue corriendo?
grep "mejorado\|✓\|ERROR" logs/<script>_run.log | wc -l  # conteo rápido
```

**¿Cómo sabe Claude cuándo terminó?** No automáticamente (nohup desacopla
el proceso). El asistente programa un `ScheduleWakeup` para volver a
revisar el log en ~20 minutos. Si el proceso terminó, lee el resultado y
reporta. Si sigue corriendo, programa otro wakeup.

**Qué NO hacer:**
- ❌ `python script.py` — muere si se cierra la terminal o el contexto de Claude.
- ❌ `python script.py &` sin nohup — muere con el shell padre al cerrar.
- ❌ `... | tee log.log` — el pipe introduce buffering, el log no se escribe
  en tiempo real y puede quedar vacío si el proceso se corta.

**Scripts afectados** (todos los de red/subprocess de larga duración):
`fetch_better_covers.py`, `mirror_images.py`, `upgrade_image_resolution.py`,
`backfill_prh_covers.py`, `upscale_images.py`, `wayback_recover.py`,
`backfill_metadata.py`, `translate_descriptions.py`, `search_discovery.py`,
`backfill_animeclick_details.py`.

Scripts rápidos (<30s, compute-only) no necesitan nohup: `rescore.py`,
`clean_titles.py`, `filter_non_manga.py`, `filter_collectible.py`,
`generate_slugs.py`, `backfill_cluster_key.py`.
...
1. **Items con ISBN** → lookup determinístico en Amazon CDN (`m.media-amazon.com/images/P/{isbn10}.09.SCLZZZZZZZ.jpg`) + PRH CDN para publishers EN (`978-0/978-1`). Sin cambios.
2. **Items sin ISBN** (y sin `variant_cover`/`retailer_exclusive`) → **Tavily Search API** (`https://api.tavily.com/search`) con `include_images: True`. Retorna hasta 5 URLs de imagen directas ya filtradas desde los resultados de búsqueda — sin fetchar HTML de cada página. Requiere `TAVILY_API_KEY` en `.env` (auto-detectado via `_load_dotenv()`). Plan gratuito: 1 000 queries/mes.

**Por qué no DDG**: `html.duckduckgo.com/html/` bloqueado (HTTP 202 → redirect a home). Alternativas probadas también fallaron: `lite.duckduckgo.com`, DDG Instant Answer API, SearXNG (403), Bing (cuerpo vacío). Tavily es la única opción como API pública real sin bloqueos.

**Nuevo helper `_load_dotenv()`**: carga `.env` desde la raíz del proyecto usando solo stdlib (`open` + `os.environ`), igual que el patrón en `search_discovery.py`. El key se auto-detecta sin que el usuario necesite exportarlo.

**Nuevo flag `--tavily-key`**: permite pasar la key explícitamente. Si no se pasa, `run()` auto-carga desde env. Si tampoco está en env, solo se procesan items con ISBN (sin mensaje de error, solo un aviso).

**Verificación de identidad de portada:** sin cambios — aspect ratio (±25%) + aHash 8×8 Hamming ≤ `--max-hash-dist`.

**Panel de Control actualizado:** preset "Todo (ISBN + DDG search)" → "Todo (ISBN + Tavily search)"; flag `--no-search` help actualizado; nuevo flag `--tavily-key` (advanced). Documentado en `scripts/retrofit/README.md` y CLAUDE.md file map.

---

Last updated: 2026-05-28 (Rakuten CDN `?_ex=NxN` fix — 9,020 imágenes mejoradas, Sumikko 2,619 portadas ×36 más píxeles) — Dos archivos actualizados para detectar y eliminar el parámetro de redimensionado de Rakuten Books CDN:

**`scripts/image_store.py::normalize_image_url`** — nueva rama #5 que detecta URLs de `thumbnail.image.rakuten.co.jp` con query param `?_ex=NxN` (ej. `...cabinet/9312/2100014729312.jpg?_ex=200x200`). Sin el parámetro, Rakuten retorna la imagen en resolución completa (tipicamente 988×1200 vs 164×200 con el param — ×36 más píxeles). Constantes agregadas: `_RAKUTEN_THUMB_HOSTS`, `_RAKUTEN_EX_RE`. Idempotente: si la URL ya está limpia, se devuelve sin cambios.

**`scripts/retrofit/upgrade_image_resolution.py::derive_original_url`** — mismo patrón #5 agregado a la función de detección. Docstring actualizado con la descripción del patrón Rakuten y el ejemplo de mejora.

**Corrida del retrofit completo**: `upgrade_image_resolution.py --workers 6` procesó 9,911 candidatas → **9,020 mejoradas** (891 sin mejora o descarga fallida, 0 errores). La mayoría del salto viene del patrón Rakuten: los ~2,717 items de Sumikko (JP - Sumikko 限定版・特装版) usan Rakuten CDN para una parte de sus portadas — ahora solo quedan 23 con imagen chica vs ~2,642 antes.

---

Last updated: 2026-05-28 (campo `rarity` — classify + retrofit + dashboard filters) — Implementación completa del sistema de rareza en 4 tiers:

**Clasificación (10,084 items):** 1,425 common (14.1%) · 8,343 rare (82.7%) · 232 super_rare (2.3%) · 84 ultra_rare (0.8%). Los 1,636 ambiguous editions (artbooks, boxsets, lore_editions sin señal `limited`) fueron clasificados por 55 agentes de búsqueda web en paralelo (batches 000–054). Los 3,998 no-ambiguous se clasificaron por reglas determinísticas. Merged en `/tmp/rarity_results/rarity_map.json` y aplicado a `items.jsonl`.
...
**Dashboard filtros:** `web/index.html` — nuevo bloque "Rareza" con 4 chips multi-select (⬜ Common, 🟦 Rare, 🟪 Super Rare, 🟨 Ultra Rare). `web-next/` — filtro "Rareza" en `SidebarFilters` con los mismos 4 chips (colores del design system PandaWatch). `FilterParams` + `parseFilterParams` + `filterClusters` actualizados. Badge de rareza en `EditionCard` (top-left, glassy dark, SVG icon + label). ScoreBadge removido de las cards (número verde, no era parte del design system final). `Item.rarity` y `FilterParams.rarity` añadidos a `web-next/lib/types.ts`.

**Distribución de calidad post-todas-las-mejoras:**
...
Last updated: 2026-05-28 (upscale_images.py — AI upscaling de portadas pixeladas + Amazon CDN normalize) — Tres mejoras de calidad visual para portadas que no tienen versión hi-res en el servidor:

**1. `scripts/retrofit/upscale_images.py`** — nuevo retrofit que aplica AI upscaling ×2 a portadas con menos de 200 000 píxeles (≈450×445 px). Auto-detecta `waifu2x-ncnn-vulkan` (preferido, optimizado para anime/manga) o `realesrgan-ncnn-vulkan` (alternativa). Procesa en serie (la GPU paraleliza internamente). Guarda el resultado como PNG lossless en `data/images/`; actualiza `image_local` en items.jsonl si la extensión cambia `.jpg → .png`. Idempotente. Registrado en Panel de Control (3 presets: dry-run, test 20, todo). Requiere instalar primero: `brew install waifu2x-ncnn-vulkan`.

**2. Amazon CDN size modifiers en `normalize_image_url`** (`scripts/image_store.py`) — añadido el patrón #4 que elimina los modificadores embebidos en el path Amazon (`._SY300_.`, `._SL165_.`, `._SS120_.` etc.) para los hosts `m.media-amazon.com` y similares. Garantiza que scrapes futuros descarguen la imagen en resolución completa directamente.

**3. Mismo patrón en `upgrade_image_resolution.py::derive_original_url`** — para que los ~14 items existentes con URLs Amazon que tienen modificadores embebidos se corrijan al próximo run del retrofit.

---

Last updated: 2026-05-28 (translate_descriptions.py completo + fallback description_es en ambas UIs) — Dos cambios en la misma sesión:

**1. Traducción completa del corpus** — `scripts/retrofit/translate_descriptions.py` procesó los 10,083 items con `description`: 9,201 traducidos al español, 882 ya estaban en español (marcados con `description_es=""` como sentinel). Cobertura final **100%**. El script acumula tres mejoras implementadas esta sesión: (a) flush incremental cada 50 items (`--flush-every`) para no perder progreso si el proceso se interrumpe; (b) limpiador de junk pre-traducción (`_clean_description_for_translation`) que quita botones de UI capturados por el scraper — "Aggiungi al Carrello" Panini IT (~128 items), "EN SAVOIR PLUS" Meian FR (~16 items); (c) sentinel por presencia de key (`"description_es" not in item`) en vez de por valor, lo que evita que items con descripción ya en español queden eternamente en la cola de pendientes. El script se lanzó con `nohup` desde el terminal del usuario para sobrevivir resets de sesión de Claude Code. `script_registry.py` actualizado con el flag `--flush-every`.

**2. Fallback `description_es || description` en ambas UIs** — El patrón i18n estándar se implementó en los tres puntos de render de descripción:
- `web/index.html` — modal descripción principal: `selectedItem.description_es || selectedItem.description`
- `web/index.html` — modal extras: `ex.description_es || ex.description`
- `web-next/components/item/ItemHero.tsx` — item detail: `canonical.description_es || canonical.description`
- `web-next/components/item/ExtrasSection.tsx` — ya tenía el fallback correcto desde antes.

Los campos originales (`description`, `extras[].description`) nunca se modifican — los usa `detect_signals()` para calcular `signal_types`. Documentado en `docs/web-html/PRD.md` (nueva sección "Convención de idioma en descripción") y `docs/web-next/FRD-005-item-detail.md` (nuevo FR-5 "Description", FR-6 "Extras" actualizado, FRs 6→10 renumerados).

---

Last updated: 2026-05-28 (backfill_prh_covers.py + image_store normalize + fix 298 items) — Tres mejoras encadenadas de calidad de imágenes EN:

**1. `scripts/retrofit/backfill_prh_covers.py`** — nuevo retrofit que mejora portadas de items EN vía CDN determinístico de Penguin Random House (`images.penguinrandomhouse.com/cover/{isbn13}`). Para items con ISBN-13 de prefijo 978-0/978-1, prueba esa URL; si devuelve imagen válida ≥80 000 px (guard de placeholders) y es mejor que la actual (--min-gain 0.10), la adopta. Deduplicado por ISBN: un download sirve a todos los items del cluster. Registrado en Panel de Control (3 presets). **Corrida inicial: 39 candidatos → 18 mejorados** (Yen Press Calendar vols Fruits Basket / Spice and Wolf / Vanitas / Übel Blatt; VIZ; Manga-Sanctuary EN), 21 sin mejora (PRH 404 o misma resolución). Idempotente.

**2. `scripts/image_store.py::normalize_image_url`** — nueva función (y actualización de `image_stem`/`download_image`) que normaliza los 3 patrones de CDN resize antes de hashear/descargar. Garantiza que future scrapes siempre descarguen la portada en resolución completa desde el primer scrape, sin necesidad de correr `upgrade_image_resolution.py` después.

**3. Fix 298 items con metadata desincronizada** — tras la corrida de `upgrade_image_resolution.py`, 298 items tenían `image_url`/`image_local` apuntando a la URL antigua con parámetros CDN (miniatura en `_orphans/`) mientras los archivos hi-res ya existían en `data/images/` bajo el hash de la URL limpia. Script de reparación one-shot los sincronizó usando `normalize_image_url` + `image_stem` para localizar el archivo correcto. Belt-and-suspenders fix también aplicado en `_apply_upgrade` de `upgrade_image_resolution.py` para que el bug no se repita.

---

Last updated: 2026-05-28 (ImageCarousel dedup + upgrade_image_resolution.py — portadas en alta resolución) — Dos mejoras de calidad visual en la misma sesión:

**1. Carrusel sin imagen duplicada** (`web-next/components/item/ImageCarousel.tsx`): el primer frame del carrusel (Portada) aparecía idéntico al primer frame de Galería porque muchos retailers guardan la cover con `?quality=80&width=222&height=222` en `image_url` y la misma URL limpia en `images[0].kind=gallery`. La función `dedupeImages()` normaliza cada URL quitando query params y fragment, usa la primera aparición (que tiene el `kind` correcto: `cover`) y toma prestado el `local` file de cualquier duplicado que lo tenga. El resultado: 1 sola imagen cuando son iguales, carrusel real cuando son genuinamente distintas. La función también hace cover `images: rawImages` → `images = dedupeImages(rawImages)` internamente para no romper la firma pública del componente. `WO-007.md` actualizado.

**2. `scripts/retrofit/upgrade_image_resolution.py`** — nuevo retrofit que sube la resolución del espejo local de portadas:
- Detecta 3 patrones de CDN que redimensionan imágenes: **Magento** (`?quality=80&width=222&height=222` → sin params; afecta ~1121 items Panini IT/ES), **WordPress/WooCommerce** (`image-300x300.jpg` → `image.jpg`; afecta ~5582 items dominados por Mangavariant 5554), **Shopify** (`image_540x.jpg` → `image.jpg`; 2 items).
- Para cada candidata: descarga la URL sin parámetros, extrae dimensiones de los bytes raw (JPEG SOF markers, PNG header, GIF, WebP VP8 — sin Pillow), descarta si la ganancia es <10% (--min-gain default). Si mejora: reemplaza el archivo local en `data/images/`, actualiza `image_url` e `images[i].url` en items.jsonl. El viejo archivo (miniatura) queda como orphan para el próximo GC de `mirror_images.py`.
- **Run inicial**: 6,877 URLs candidatas detectadas, todas mejoradas (Mangavariant y Panini IT sirven siempre la imagen completa al quitar los parámetros; la validación de píxeles confirmó mejora real en todos los casos procesados).
- Registrado en `script_registry.py` (categoría Mantenimiento, 3 presets). Documentado en `scripts/retrofit/README.md` y en este archivo.
- Idempotente: URLs ya limpias no producen `derive_original_url()` distinto → no se reprocesan.
