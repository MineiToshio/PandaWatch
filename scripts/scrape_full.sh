...
#   RUN_COVER_BACKFILL=1         # añade [4g3]/[4g4] backfill_prh_covers + fetch_better_covers
#                                #   (mejora de portadas vía CDN PRH + búsqueda web; network-heavy,
#                                #   fetch_better_covers necesita SERPER_API_KEY o TAVILY_API_KEY
#                                #   en .env para buscar. Default OFF — ver P25.)
...
# hermanos `items.jsonl.*.pre-scrape-full-bak`, se conservan los 3 más recientes).
...
if [ -s data/items.jsonl ]; then
    ...
    PRESCRAPE_BACKUP=$(env PYTHONUNBUFFERED=1 "$VENV_PY" -u -c \
        "import sys; sys.path.insert(0,'scripts'); from pathlib import Path; from manga_watch import backup_and_rotate; print(backup_and_rotate(Path('data/items.jsonl'), 'scrape-full', timestamped=True))")
...
if [ "$SKIP_SCRAPE" != "1" ]; then
    ...
    if [ "$P1_RC" -eq 124 ]; then
        echo "    ⚠ Fase 1 alcanzó el TIMEOUT (3 h, rc=124) — se preservó lo ya flusheado por fuente."
    fi
...
if [ "$SKIP_CLEANUP" != "1" ]; then
    ...
    # [4e]/[4e2]/[4e3] hacen 1 request HTTP por item pendiente — sin _run_timed
    # antes (regresión de gotcha #33, S3 auditoría 2026-07-08): un host colgado
    # bloqueaba el run entero (horas, en full) y mantenía el lock global tomado.
    echo ">>> [4e] backfill_metadata --only image_url"
    P4E_START=$(date +%s)
    _run_timed 3600 "$VENV_PY" scripts/retrofit/backfill_metadata.py --only image_url --sleep 0.5 \
    ...
    P4E2_START=$(date +%s)
    _run_timed 3600 "$VENV_PY" scripts/retrofit/backfill_metadata.py --only images --sleep 0.5 \
        > "$LOG_DIR/04e2-backfill-galleries.log" 2>&1
    record_step "backfill_metadata:images" $?
    echo "    duración: $(($(date +%s) - P4E2_START))s — items: $(count_lines)"

    echo ">>> [4e3] mirror_images --no-gc (descarga gallery images al espejo local)"
    P4E3_START=$(date +%s)
    _run_timed 3600 "$VENV_PY" scripts/retrofit/mirror_images.py --no-gc --workers 8 \
        > "$LOG_DIR/04e3-mirror-galleries.log" 2>&1
    record_step "mirror_images:galleries" $?
    echo "    duración: $(($(date +%s) - P4E3_START))s — items: $(count_lines)"
    ...
    if [ "${RUN_COVER_BACKFILL:-0}" = "1" ]; then
        ...
        P4G3_START=$(date +%s)
        _run_timed 1800 "$VENV_PY" scripts/retrofit/backfill_prh_covers.py --workers 8 \
            > "$LOG_DIR/04g3-backfill-prh-covers.log" 2>&1
        record_step "backfill_prh_covers" $?
        echo "    duración: $(($(date +%s) - P4G3_START))s — items: $(count_lines)"

        echo ">>> [4g4] fetch_better_covers (búsqueda web de portadas hi-res → preview)"
        P4G4_START=$(date +%s)
        _run_timed 3600 "$VENV_PY" scripts/retrofit/fetch_better_covers.py \
            > "$LOG_DIR/04g4-fetch-better-covers.log" 2>&1
        record_step "fetch_better_covers" $?
        echo "    duración: $(($(date +%s) - P4G4_START))s — items: $(count_lines) (candidatas nuevas en data/cover_preview.json, si las hay)"
    else
        echo "    [SKIP] backfill_prh_covers + fetch_better_covers (RUN_COVER_BACKFILL=0; opt-in, network-heavy)"
    fi
