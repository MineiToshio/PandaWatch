...
| `rare` | Print run > 2500 y confirmado agotado; o `retailer_exclusive` sin verificar; o tokuten; o keyword de no-reimpresión (`_SINGLE_RUN_KEYWORDS`: incluye 限定版, 特装版 y similares; se removió la familia genérica "limited edition" para evitar FP); o patrón de tirada única (`_SINGLE_RUN_PATTERNS`) |
