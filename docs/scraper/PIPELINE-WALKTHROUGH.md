...
7. **Flush incremental** — escribe tras CADA fuente (resiliencia). **Desde el paquete G
   (2026-07-08)** esto ya NO es un `append_jsonl` completo (O(corpus)) por fuente: escribe
   a un spool append-only (`items.jsonl.spool`) y es el paso 10 (`append_jsonl` FINAL del
   run) el que absorbe el spool entero en una sola pasada (ver "Convenciones de ambos
   scripts" arriba) — el paso 10 corre UNA vez al final del run, no por-candidato.
8. **Image mirror** `mirror_candidate_images()` — descarga CADA imagen a `data/images/<sha256>.<ext>` y setea su `local` en `images[]` (`images[0]` = portada). `_extract_images_from_detail_soup` trae el carrusel a `images[]`. (Ya no hay campos top-level `image_url`/`image_local`; el `Candidate` runtime los lleva como input y `candidate_to_json` los vuelca en `images[0]`.) **Desde 2026-06-15 `download_image` ESTANDARIZA cada imagen al bajarla** (AVIF Q60, lado largo ≤1600px, sin metadata; fuente única `image_store.normalize_image`) → el espejo entra ya optimizado. Solo achica (el upscale es aparte); los placeholders pasan crudos para no romper la detección por firma (gotcha #104).
9. **`derive_cluster_key`** — tiers `lmc:` > `edition:` > `fuzzy:` > `url:`. Se deriva DESPUÉS de escribir el edition_key heurístico en el row (gotcha #65): la fila fresca entra ya consistente con la invariante CLKEY.
