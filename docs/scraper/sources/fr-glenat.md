...
> ⚠️ **Split 2026-07-07**: la entrada `"FR - Glénat Art Books"` se documenta ahora
> aparte, en [fr-glenat-artbooks.md](fr-glenat-artbooks.md) — el sitio migró a
> Next.js con hidratación client-side y esa entrada pasó a `kind: js` (esta ficha
> sólo cubre la línea manga, que sigue siendo `kind: html`).
...
| **Índice / punto de entrada** | `/manga/nouveautes/` (novedades) |
| **Tipo de fuente** | Editorial (official) |
| **`kind` en sources.yml** | `html` |
| **`source_class`** | `official` |
| **País(es)** | Francia (`Francia`) — el país va al edition_key |
| **Idioma(s)** | Francés (FR) |
| **Cobertura** | Novedades de la línea manga de la editorial francesa Glénat |
| **Aporte al corpus** | 2 items (al último corpus) |
| **Parser / módulo** | Entrada en `sources.yml` (extractor genérico HTML; sin parser propio) |

**"FR - Glénat Manga Nouveautés"** → `https://www.glenat.com/manga/nouveautes/`,
`max_pages: 15`, tags `["manga", "official", "france"]`, `publisher: Glénat Manga`,
`country: Francia`, `source_class: official`, `kind: html`.

Los **art books** de Glénat (`"FR - Glénat Art Books"`) tienen su propia ficha —
ver [fr-glenat-artbooks.md](fr-glenat-artbooks.md) — porque desde 2026-07-07 requieren
`kind: js` (el listado se renderiza client-side).

**Por qué importa / qué aporta de único**: es la voz **oficial de la editorial** para el
mercado **francés** (FR), uno de los mercados clave de ediciones premium de manga.

---

## 2. Descripción técnica de la fuente

- **Estructura de URLs / páginas**: listado de novedades paginado (hasta `max_pages: 15`).
  Cada producto enlaza a su ficha en `glenat.com`.
...
- **Entrada en `sources.yml`**: `"FR - Glénat Manga Nouveautés"` descrita en §1.
- **Flujo end-to-end**: entra en la **FASE 1** de `scrape_full.sh` / `scrape_delta.sh`
  (scrape de las sources del YAML vía `manga_watch.py --workers 8`). Luego pasa por los
